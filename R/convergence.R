...
data1.0<-fread('~/Documents/featurecloud/singular-value-decomposition//results/matrix/summary.angles.u.tsv')
...
data2.0<-fread('~/Documents/featurecloud/singular-value-decomposition//results/vector//summary.angles.u.tsv')
...
offset<-data[Algorithm=='gradient'] %>% group_by(eigenvector, sites, Algorithm)%>% summarise(offset=max(iteration)) %>% summarise(mean_offset = floor(mean(offset)))
...
data2$iteration<-rowSums(data2[, c("index", "mean_offset")], na.rm = T)
...
time <- fread('~/Documents/featurecloud/singular-value-decomposition//results/time.log')
time$V1<-recode(time$V1, 'qr_scheme_power-approx'='AI-FULL', 'qr_scheme_power-iteration'='RI-FULL', 'randomized'='RAND', 'randomized-approx-projected'='AI-RAND', 'guo_single_gradient'='GUO')
time<- time[V1 %in%  c('AI-FULL', 'RI-FULL', 'RAND', 'GUO')]
...
eog<- fread('~/Documents/featurecloud/singular-value-decomposition/results/matrix/summary.eo.tsv')
eo<-fread('~/Documents/featurecloud/pca/approximative-vertical/results/vector/summary.eo.tsv')
eo<-eo[, c('V1', 'V2', 'V3', 'V4', 'V6'), with=F]
eo<-rbind(eo, eog, use.names=F)
eo$V4<-recode(eo$V4, 'power-approx'='AI-FULL', 'power-iteration'='RI-FULL', 'randomized'='RAND', 'guo'='GUO')
eo<- eo[V4 %in% c('AI-FULL', 'RI-FULL', 'RAND', 'GUO') ]
eo <- eo %>% group_by(V3,V4, V6) %>% summarise(sum=sum(V2))
...
transmission<-fread('~/Documents/featurecloud/singular-value-decomposition/results/matrix/summary.transmission.tsv')
transmission_guo<-fread('~/Documents/featurecloud/singular-value-decomposition/results/vector/summary.transmission.tsv')
