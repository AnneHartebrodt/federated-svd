...
x = '/home/anne/Documents/featurecloud/singular-value-decomposition/results/mnist/'
...
scales_x <- list(
  ...
  `Eigenvector 5` = scale_x_continuous(limits = c(1, 120)),
...
ggsave(file=paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/plots/convergence_mnist.pdf'), width = 18, units = 'cm', height = 8)
```
```{r, echo=FALSE, warning=FALSE}
...
pl.eo<-ggplot(eo, aes(V4, sum, fill=interaction(V4,V3), color=V4))+geom_boxplot()+ylab('Runtime[s] (Matrix oper.)')+xlab('')+
  ...
  scale_fill_manual(values=c(viridisLite::magma(4, end=0.7, alpha = 0.6), viridisLite::magma(4, end=0.7, alpha = 0.8)))
...
```{r, echo=FALSE}
ggsave(file=paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/plots/benchmark_mnist.pdf'), width = 18, units = 'cm', height = 7.5)
```
...
scales_x <- list(
  `Eigenvector 1` = scale_x_continuous(limits = c(1, 11)),
  `Eigenvector 5` = scale_x_continuous(limits = c(1, 100)),
...
chr <- '2'
ob<-combine_data(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/chr',chr,'.100000/vertical/'))
data<-ob[[2]]
iterations<-ob[[1]]
data$SNPs<-'100000'
iterations$SNPs<-'100000'

ob1<-combine_data(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/chr',chr,'.500000/vertical/'))
data1<-ob[[2]]
data1$SNPs<-'500000'
iterations1<-ob1[[1]]
iterations1$SNPs<-'500000'

ob2<-combine_data(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/chr',chr,'.all/vertical/'))
data2<-ob[[2]]
data2$SNPs<-'>1Mio'
iterations2<-ob2[[1]]
...
conv<-ggplot(data[eigenvector %in% c('Eigenvector 1', 'Eigenvector 5', 'Eigenvector 10')], 
  ...
  theme(legend.position = 'bottom',
        strip.text = element_text(size=8), panel.border = element_rect(fill=NA), 
        panel.spacing = unit(0.05, 'cm'),strip.background = element_blank())+ # remove sites strip label
  scale_color_viridis_d(option = 'A', end=0.7)+extra_theme
...
for (sub in c(paste0('chr',chr,'.100000'), paste0('chr',chr,'.500000'), paste0('chr',chr,'.all'))){
  eog<- fread(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/',sub,'/vertical/matrix/summary.eo.tsv'))
  eo<-fread(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/',sub,'/vertical/vector/summary.eo.tsv'))
  eo<-rbind(eo, eog, use.names=F)
...
pl.eo<-ggplot(eo, aes(V4, sum, fill=interaction(V4,SNPs), color=V4))+geom_boxplot()+ylab('Runtime[s] (Matrix oper.)')+xlab('')+
  theme_classic()+
  extra_theme+
  guides(fill=FALSE, color=F)+
  scale_color_viridis_d(option = 'A',4, end=0.7, begin=0.2, alpha = 1)+
  scale_fill_manual(values=c(viridisLite::magma(4, end=0.7, begin=0.2, alpha = 0.2), viridisLite::magma(4, end=0.7, begin=0.6, alpha = 0.4), viridisLite::magma(4,end=0.7, begin=0.2, alpha = 0.8)))
...
iterations$Algorithm<-recode(iterations$Algorithm, 'power-approx'='AI-FULL', 'power-iteration'='RI-FULL', 'randomized'='RANDOMIZED', 'gradient'='GUO')
iterations <- iterations[Algorithm %in% c('AI-FULL', 'RI-FULL', 'RANDOMIZED', 'GUO') ]
pl.iter<-ggplot(iterations, aes(Algorithm, maxit, fill=interaction(Algorithm, SNPs),color=Algorithm))+
  geom_boxplot()+
  xlab('')+
  theme_classic()+
  ylab('Iterations to convergence')+
...
tol<-list()
for (sub in c(paste0('chr',chr,'.100000'), paste0('chr',chr,'.500000'), paste0('chr',chr,'.all'))){
  transmission<-fread(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/',sub,'/vertical/matrix/summary.transmission.tsv'))
transmission_guo<-fread(paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/1000g/',sub,'/vertical/vector/summary.transmission.tsv'))
transmission<-rbind(transmission_guo, transmission)
transmission<-transmission[V2 %in% c("H_local=CS", "H_global=SC")]
colnames(transmission) <- c('index', 'key', 'iteration', 'site', 'size', 'kA', 'sites', 'algorithm', 'filename')
transmission<-transmission[site==1]
...
volume$algorithm<-recode(volume$algorithm, 'power-approx'='AI-FULL', 'power-iteration'='RI-FULL', 'randomized'='RANDOMIZED', 'gradient'='GUO')
volume<- volume[algorithm %in% c( 'RI-FULL', 'AI-FULL', 'RANDOMIZED', 'GUO')]
pl.volume<-ggplot(volume, aes(algorithm, total_volume, fill=interaction(algorithm, SNPs),color=algorithm))+geom_boxplot()+
  ylab('Total transmitted\n data [GB]')+
  xlab('')+
  theme_classic()+
  extra_theme+
...
```{r, echo=FALSE}
############FAKE ALPHA LEGEND
pl.volume.legend<-ggplot(iterations, aes(Algorithm, maxit, fill=Algorithm, alpha=as.factor(SNPs)))+geom_boxplot()+
  theme_classic()+ylab('Total transmitted data [GB]')+xlab('')+
  scale_alpha_manual(values=c(0.2,0.4, 0.7))+
  # Use this to create a legens for splits category
  #guides(fill=F,alpha=guide_legend('#Sites', override.aes=list(fill=hcl(c(15,195),100,0,alpha=c(0.2,0.7)))))
  guides(fill='none',alpha=guide_legend('#SNPs', override.aes=list(fill=hcl(c(15),100,0,alpha=c(0.2)))))
...
```{r, echo=FALSE}
ggsave(file=paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/plots/benchmark_chromosome_',chr,'.pdf'), width = 18, units = 'cm', height = 7.5)
```
