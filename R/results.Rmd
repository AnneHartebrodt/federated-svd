...
scales_x <- list(
  ...
  `Eigenvector 5` = scale_x_continuous(limits = c(0, 200)),
  `Eigenvector 10` =scale_x_continuous(limits = c(0, 750))
...
iterations$SNPs<-as.factor(iterations$SNPs)
```
...
pl.volume.legend<-ggplot(iterations, aes(Algorithm, maxit, fill=Algorithm, alpha=as.factor(SNPs)))+geom_boxplot()+
  ...
  # Use this to create a legens for splits category
  #guides(fill=F,alpha=guide_legend('#Sites', override.aes=list(fill=hcl(c(15,195),100,0,alpha=c(0.2,0.7)))))
  guides(fill=F,alpha=guide_legend('#SNPs', override.aes=list(fill=hcl(c(15),100,0,alpha=c(0.2)))))
...
# Convert the plot to a grob
gp
ggsave(file=paste0('/home/anne/Documents/featurecloud/singular-value-decomposition/results/plots/chromosome_',chr,'.pdf'), width = 18, units = 'cm', height = 6.5)
```
